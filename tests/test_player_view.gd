...
func _defaults(t: RefCounted, tree: SceneTree) -> void:
    ...
    t.eq(play._hud.get_theme_font_size("font_size"), 18, "player HUD font 18 px (readable at 720p)")
    _drop(play)
    ...
    t.eq(dev._hud.get_theme_font_size("font_size"), 15, "--view=dev: HUD font 15 px as before")
    _drop(dev)
