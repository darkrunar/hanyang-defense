...
- 작성일: 2026-09-14
- WP / 상태: WP-002 Bongsu Network / **DONE** (1차 GPT REVISE `8ddab9d` → 보완 `9c304c6` → 2026-09-14 GPT 재리뷰 **PASS 8/0/0** `869788b`. DONE 전환 2026-09-14, Claude Code)
...
| AC-07 | **PASS** | 새 시설 배치·단절 상태 읽기·조작 | 봉수대를 골목에 설치해도 경로 버전 불변·통행 유지; 센서 겹침 거절, 비활성 후에도 겹침 거절(점유 유지), 지형·경계·적 점유 거절, 미리보기 일치, 제거 후 재설치, 알 수 없는 id 거절. 화면: 간선·부착선·탐지/로컬/연결 반경·그룹 태그·비활성 회색, 키 3/4/T와 안내 줄. 1차 제출 캡처에는 커서 패널이 없었다(캡처 모드 show_cursor=false) — 보완 회차 캡처 a2/a4는 스크립트 hover로 패널을 포함 | 테스트 "AC-07" 17건; 캡처 5장(HUD·반경 표시, 보완 회차부터 패널 포함) |
...
1. **P-010 fixture A 로컬 사격 기하(GPT 판단 요청)** — H(46,29)=(940,600)과 가장 가까운 후보 영역 원 가장자리(Z0/Z1)까지 110.24px(sqrt(40²+150²)−45; 1차 제출의 112px은 근삿값)라 로컬 100px 안이면서 후보 영역 안인 적이 존재할 수 없다. 계약 4단계는 보조 화차 H2(48,41)=(980,840)로 입증했다(Z1 90px 로컬, fixture 적 120px 비로컬). 대안: 현행 수용 / 광장 남단 후보 영역 추가(fixture B "8개 영역"·WP-001 모드 보존에 영향) / fixture A의 H 위치 변경.
...
## 보완 회차 · 2026-09-14 (GPT REVISE R-01 / R-02 / R-03 반영, Claude Code)

1차 제출(구현 `efa53bb`, 결과 `8471809`)과 1차 GPT 리뷰(`8ddab9d`, 아래 GPT Review 절)를 보존하고, 보완 구현 커밋 **`9c304c6`** 기준으로 다시 검증했다. 채택 증거는 같은 경로에 두고, 이전 회차 성능 원시 파일은 접미사를 붙여 보관했다(`*_run1_pv_bookkeeping`, `*_run2_no_candidate_eval` / `*_run2_no_event_log`, `*_run3a_sha_tag`).

### 변경 내용 (`8ddab9d` → `9c304c6`)

| 파일 | 변경 | 대응 |
|---|---|---|
| `tests/test_bongsu_network.gd` | 중복 센서·순환 fixture의 앵커를 열린 셀로 교체 — S2 (48,37)=(980,760), B2 (48,33)=(980,680), B3 (46,27)=(940,560) — 하고 **모든 설치 ok·센서/봉수대/간선 수·단일 그룹·센서별 동일 관측 집합을 비교 전에 단언**. 피해 적용 횟수(`damage_applications`)도 비교. variant 1/2/3 = 센서 1·봉수대 1·간선 0 / 센서 2·봉수대 1·간선 0 / 센서 2·봉수대 3·간선 3(삼각) | R-01 |
| `game/core/enemy_sim.gd` | `damage_applications` 집계(볼리당 적 1회) | R-01 |
| `game/core/hwacha.gd`, `battle.gd` | `Hwacha.evaluate_only()`: 전투 비활성에서도 활성 화차마다 `known_zone_counts` + `select_zone`을 매 틱 실행(발사·피해·쿨다운 없음). `candidate_evaluations`를 사격 경로와 평가 경로 양쪽에서 집계 | R-02 |
| `tests/test_bongsu_network.gd` | R-02 회귀: fixture B 전투 비활성 5초 → 평가 횟수 = 틱 × 활성 화차(300×4=1200), 발사 0·피해 0·처치 0·쿨다운 0, 화차가 망으로 적을 인지하고 표적 후보를 선택 | R-02 |
| `game/scenes/main.gd` | network_combat **이벤트 로그**: 명령별 측정 상대시각·sim 시각·결과·B8 id/active 전후·S3 attached/group 전후·path/topology 전후·재시도 횟수·거절 사유. **성공한 명령만 집계**(B8 전환·장승 제거는 반환값 확인). **측정 시작 스냅샷**과 측정 구간 델타(발사·공유전용·처치·평가 횟수·sim 초), 구간 내 공유전용 사격 샘플(시각·화차·영역·로컬/공유·처치, 상한 300), **manifest**(구현 SHA `--sha`, 전체 설정, fixture B 좌표, 장승 앵커, B8/S3 id, 영역표). 캡처 시나리오에 스크립트 hover(a2: H, a4: H2) | R-03, AC-07 |
| `scripts/perf_with_memory.ps1`, `verify.ps1`, `verify.sh` | `--sha=<git HEAD[-dirty]>` 전달(dirty는 game/·project.godot·export_presets.cfg 기준), 계약 검사에 측정 구간 평가 횟수 >0·B8 성공 이벤트 12·구간 내 공유전용 사격 ≥1 추가 | R-02, R-03 |
| `docs/DECISIONS.md`, `backlog/WP-002.md`, 이 문서 | P-010 승인 반영(H2 (48,41), 원 경계 최단 110.24px), fixture A 4단계 주석, AC-07 캡처 문구 정정 | P-010 |

### AC 재검증 (보완 회차, 구현자 보고)

| AC | 보완 회차 | 실제 결과 | 증거 |
|---|---|---|---|
| AC-01·02·03·04·07·08 | PASS (변경 없음) | 재실행 동일. 캡처 재생성(a2·a4에 hover 패널 포함) | `wp-002/tests/test_report.txt`, `wp-002/captures/` |
| AC-05 | **PASS (R-01 수정)** | variant 1/2/3 설치 전부 ok, 간선 0/0/3, 그룹 1, S1·S2가 같은 6개체 id 관측. 세 variant 모두 **인지 6 · 영역 밀도 6 · 2초 발사 2 · 처치 6 · 피해 적용 12** 로 동일(증폭 없음). 로컬+공유 중복 1회, 앞 화차 처치 제외는 유지 | `test_report.txt` "AC-05" 55건 |
| AC-06 | **PASS (R-02·R-03 보완)** | 아래 표. network_move 측정 구간 평가 14,400회(=3,600틱×4화차), network_combat 5,527회(쿨다운 틱 제외). 이벤트 24건 전부 ok, B8 전환 0.01/5.01/10.00/…/55.01 s, S3 부착 12↔−1 교대, 장승 설치 6/6·제거 6/6(재시도 0), path_version 3→15 단계별 기록. **측정 구간 내 공유전용 사격 112발**(누적 123 중 준비 구간 11발 제외), 구간 발사 185·처치 4,467 | `wp-002/perf/perf_network_*_1000_release.json`(events·measure_start·measured_window·shared_only_samples·manifest), `*.memory.json` |

자동 검증: **322 passed / 0 failed** (WP-001 회귀 135 + WP-002 187).

### 성능 보완 측정 — 구현 커밋 `9c304c6`, 릴리스 빌드, 1920×1080, vsync off, 10s 준비 + 60s 측정, `benchmark_hold_alive`

| 회차 | 시나리오 | 프레임 / 초 | 생존 min / avg / max | 부하 유지 | 평균 FPS | 프레임 ms p50 / **p95** / p99 / max | sim step ms avg / p95 / max | 측정 구간 후보 평가 | 이벤트·사격(측정 구간) | 워킹셋 MB 시작→종료 (최대) | 종료 |
|---|---|---|---|---|---|---|---|---|---|---|---|
| **3b (채택)** | network_move | 11,779 / 60.01 | **1000 / 1000.0 / 1000** | true | **196.3** | 3.26 / **13.68** / 15.93 / 27.43 | 6.56 / 7.70 / 9.03 | **14,400** | 발사 0, pv 3=기대 3 | 173.6→176.3 (176.4) | 0 |
| **3 (채택, manifest SHA `9c304c6-dirty`†)** | network_combat | 15,068 / 60.00 | **1000 / 1000.0 / 1000** | true | **251.1** | 2.77 / **10.78** / 12.59 / 41.81 | 4.08 / 8.08 / 8.79 | **5,527** | B8 12/12 (이벤트 ok 12), 장승 12/12 (거절 0), pv 15=기대 15, 위상 v16→v28, 구간 발사 185 · **공유전용 112** · 처치 4,467 | 171.3→172.2 (172.2) | 0 |
| 3a (참고) | network_move | 11,768 / 60.01 | 1000 / 1000.0 / 1000 | true | 196.1 | 3.29 / 13.52 / 15.83 / 23.43 | 6.47 / 7.90 / 9.06 | 14,400 | manifest SHA가 실행 중 커밋 시점 때문에 `8ddab9d-dirty`로 기록됨(코드 트리는 9c304c6과 동일, `git diff 9c304c6 -- game project.godot export_presets.cfg` 비어 있음). SHA 태그 정리 후 3b로 대체 | 169.8→147.3 | 0 |
| 2 (1차 제출, 참고) | network_move | 15,671 | 1000 | true | 261.2 | 2.79 / 10.03 / 11.74 / 16.89 | 2.23 / 2.49 / 2.83 | 0 (R-02 미충족) | — | 172.0→176.7 | 0 |
| 2 (1차 제출, 참고) | network_combat | 15,281 | 1000 | true | 254.7 | 2.79 / 10.12 / 11.73 / 18.18 | 3.99 / 7.64 / 8.72 | 미기록 | 누적만 기록(R-03 미충족) | 172.8→177.7 | 0 |

- † 정정(GPT 2차 리뷰): network_combat 회차 3의 manifest SHA는 `9c304c6-dirty`다(당시 wrapper가 results/ 증거 변경도 dirty로 셈). 코드 트리는 9c304c6과 동일했지만 접미사만으로 이를 확정할 수 없다는 지적을 수용한다. 리뷰어가 544ef86 체크아웃에서 새로 export한 릴리스로 network_combat을 재측정해 출처를 보완했다(manifest `544ef86`, 264.1 FPS / p95 10.017 ms, alive 1000, 이벤트 24 ok, 구간 공유전용 112 — `evidence/wp-002/gpt-review/2026-09-14-followup/perf_network_combat_clean.json`). 이전 "양쪽 SHA 일치" 표현은 move에만 해당한다.
- 예산 대비: 평균 60 FPS 이상 → **196.3 / 251.1 (통과)**, p95 25 ms 이하 → **13.68 / 10.78 ms (통과)**, 매 프레임 alive ≥1000 → `alive_raw` 전 원소 1000 (통과). 원시 배열 재계산 FPS 196.3 / 251.1, p95 13.676 / 10.780 — 저장값과 일치.
- **정직 기록:** R-02 반영으로 network_move의 비용이 크게 늘었다(sim step 평균 2.23 → 6.56 ms, p95 프레임 10.03 → 13.68 ms). 전투 비활성에서는 모든 활성 화차가 매 틱 후보를 계산하는 반면(14,400회), 전투 활성에서는 재장전 중인 틱을 건너뛰어(5,527회) 전투 시나리오가 오히려 가볍다. 화차별 `known_zone_counts`는 인지 집합 × 8영역 거리 검사라 인지 적이 많은 fixture B에서 지배적 비용이다. 예산 내이지만 시설·적 수가 늘면 영역별 공간 분할이나 인지 집합의 영역 사전 분류가 필요하다(미측정, WP-003 주의점).
- network_combat p99 12.59 ms, 최대 41.81 ms 단발 스파이크 1회(원인 미측정). 60초 워킹셋 증가 ≤3 MB.

### 이 회차의 알려진 문제

- network_move 비용 증가(위 정직 기록). 최적화는 측정 근거로 후속 결정.
- 프레임 시간 이봉 분포와 단발 최대 스파이크는 미측정 유지.
- 이벤트 로그의 `t_measure`는 물리 틱 경계라 0.00이 아닌 0.01 s 등으로 기록된다(허용 오차 1틱).

### 재리뷰 결과 반영 (2026-09-14)

R-01(유효 fixture·선행 단언), R-02(이동 시나리오의 후보 계산 부하 + 새 릴리스 측정), R-03(이벤트별 이력·측정 구간 분리·공유전용 사격 샘플·manifest), P-010 문서 반영을 완료해 재리뷰를 요청했고, GPT 2차 리뷰(`869788b`)가 **PASS 8 / FAIL 0 / NOT RUN 0**으로 판정했다. 이에 따라 WP-002를 **DONE**으로 전환한다(backlog/WP-002.md, docs/ROADMAP.md, README). 리뷰가 정정한 combat manifest SHA 표기는 위 †에 반영했다. 남은 후속 항목: 이동 시나리오 후보 계산 비용(공간 분할 검토), 프레임 시간 이봉 분포·단발 스파이크 원인, P-007 예약 설치, 장기 누수·다른 장비·5,000체 미보장.
