...
### 성능 보완 측정 — 구현 커밋 `9c304c6` (manifest `implementation_sha` 일치), 릴리스 빌드, 1920×1080, vsync off, 10s 준비 + 60s 측정, `benchmark_hold_alive`
...
| **3 (채택)** | network_combat | 15,068 / 60.00 | **1000 / 1000.0 / 1000** | true | **251.1** | 2.77 / **10.78** / 12.59 / 41.81 | 4.08 / 8.08 / 8.79 | **5,527** | B8 12/12 (이벤트 ok 12), 장승 12/12 (거절 0), pv 15=기대 15, 위상 v16→v28, 구간 발사 185 · **공유전용 112** · 처치 4,467 | 171.3→172.2 (172.2) | 0 |
...
R-01(유효 fixture·선행 단언), R-02(이동 시나리오의 후보 계산 부하 + 새 릴리스 측정), R-03(이벤트별 이력·측정 구간 분리·공유전용 사격 샘플·manifest), P-010 문서 반영을 완료했다. GPT 재리뷰에서 AC-05·AC-06을 다시 판정해 주기를 요청한다. 최종 판정은 아래 GPT Review 절에 날짜별로 추가한다.
