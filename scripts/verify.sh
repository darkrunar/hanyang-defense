...
echo "== 4e/6 WP-005 art pipeline captures (greybox / sample with the dev fixture; not game assets)"
EVID5="$EVID/wp-005"
mkdir -p "$EVID5/captures" "$EVID5/perf" "$EVID5/tests"
...
for size in "" _720; do
    for label in initial before_collapse after_collapse after_recovery; do
        ...
        [[ -n "$g" && "$g" == "$s" ]] || { echo "capture wp005$size: state differs at $label"; exit 1; }
    done
done
